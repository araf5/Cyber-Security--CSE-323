<section class="panel" id="manual">
    <form id="registration_form" role="form" method="post">
        <header class="panel-heading">Add Video</header>
        <div class="row">
            <div class="form-group col-md-12">
                <label>Choose Icon</label>
                ( <span class="demo-icon"></span> )
                <input type="text" name="icon" id="icon" class="icon-class-input form-control" readonly="" style="width: 49.5%;background-color: white" value="<?php if(isset($result)){ echo $result['icon'];}else{    echo 'fa fa-music';} ?>" />
                <button type="button" class="btn btn-primary picker-button" style="width: 49.5%;float: right;margin-top: -3%">Pick an Icon</button>
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-12">
                <label>Video Heading (Character Limit-30)</label>
                <input type="text" class="form-control" xmaxlength="30" placeholder="Video Heading" name="title" id="title" required value="<?php if(isset($result)){ echo $result['title']; } ?>">
            </div>
        </div>
        
         <div class="row">
            <div class="form-group col-md-12">
                <label>Video URL (Character Limit-100)</label>
                <input type="text" class="form-control" xmaxlength="30" placeholder="Video URL" name="url" id="url" required value="<?php if(isset($result)){ echo $result['url']; } ?>">
            </div>
        </div>
            ...
            <div class="modal-body">
                <div>
                    <ul class="icon-picker-list">
                        <li>
                            <a data-class="{{item}} {{activeState}}" data-index="{{index}}"  onclick="click_icon()">
                                <span class="{{item}}"></span>
                                <span id="icon-val" class="name-class">{{item}}</span>
                            </a>
                        </li>
                    </ul>
                </div>
...
// Loop through JSON and appends content to show icons
    $(icons).each(function (index) {
        ...
        itemtemp = itemtemp.replace(/{{item}}/g, item).replace(/{{index}}/g, index).replace(/{{activeState}}/g, activeState).replace(/{{click}}/g, click);
